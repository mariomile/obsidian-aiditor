 ...
 *
 * TODO: Reading view renders block-id'd elements as plain `<span id="ai-...">`
 ...
 *   1. Locate the element Obsidian renders for a line carrying `^ai-<id>`
 *      (MarkdownPostProcessor gets the rendered `el` + a `sourcePath`; block
 *      refs show up as `<*  id="ai-xxxxxx">` on the block's own element in
 *      most cases — verify across paragraph/list/callout/table).
 *   2. Insert a `.aiditor-reading-marker` sibling/badge next to it, wired to
